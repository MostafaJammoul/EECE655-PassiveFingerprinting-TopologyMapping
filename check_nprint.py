#!/usr/bin/env python3
"""
Check nPrint dataset class distribution

Reads data/processed/nprint.csv and displays OS version distribution
...
def check_nprint_distribution(csv_path='data/processed/nprint.csv'):
    """
    Check OS version distribution in nPrint dataset

    Args:
        csv_path: Path to processed nprint CSV file
    """
    csv_file = Path(csv_path)

    if not csv_file.exists():
        print(f"ERROR: File not found: {csv_path}")
        print("\nThe nPrint dataset hasn't been preprocessed yet.")
        print("Run preprocessing first:")
        print("  python preprocess_scripts/nprint_preprocess.py <input.pcapng> data/processed/nprint.csv")
        return None

    print("="*70)
    print("nPRINT DATASET - OS VERSION DISTRIBUTION")
    print("="*70)
    print(f"\nReading: {csv_path}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Check nPrint dataset OS version distribution'
    )
    parser.add_argument(
        '--input',
        type=str,
        default='data/processed/nprint.csv',
        help='Path to nprint CSV file (default: data/processed/nprint.csv)'
    )

    args = parser.parse_args()

    df = check_nprint_distribution(args.input)
