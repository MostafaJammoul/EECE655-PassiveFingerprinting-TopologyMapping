...
def calculate_flow_features(row):
    ...
    if flow_duration and flow_duration > 0 and pkt_count:
        features['pkt_rate'] = pkt_count / flow_duration
    else:
        features['pkt_rate'] = None

    # Byte rate (bytes per second)
    if flow_duration and flow_duration > 0:
        total_bytes = get_num('bytes_sent') + get_num('bytes_received')
        if total_bytes > 0:
            features['byte_rate'] = total_bytes / flow_duration
        else:
            features['byte_rate'] = None
    else:
        features['byte_rate'] = None

    # Average packet size
    if pkt_count and pkt_count > 0:
        total_bytes = get_num('bytes_sent') + get_num('bytes_received')
        if total_bytes > 0:
            features['avg_pkt_size'] = total_bytes / pkt_count
        else:
            features['avg_pkt_size'] = None
    else:
        features['avg_pkt_size'] = None

    # Upload/Download ratio
    bytes_sent = get_num('bytes_sent')
    bytes_received = get_num('bytes_received')
    if bytes_received > 0:
...
def preprocess_masaryk(raw_dir='data/raw/masaryk',
    ...
    flow_features = ['pkt_count', 'flow_duration', 'avg_pkt_size', 'pkt_rate']
    available_flow_features = [f for f in flow_features if f in df.columns]
